...
volatile unsigned long brojac90s;
...
void PID_brzinski(void)
{
	...
	if(Ie_brzina_R < -200)
		Ie_brzina_R = -200;
//dodat sample 16 tj. oduzet
	PID_ukupni_L = ((float)(Pe_brzina_L) * Kp_brzina + (float)(Ie_brzina_L) * Ki_brzina + (float)(De_brzina_L) * Kd_brzina)-sample_L16; // znak - je zbog smera kretanja
	PID_ukupni_R = ((float)(Pe_brzina_R) * Kp_brzina + (float)(Ie_brzina_R) * Ki_brzina + (float)(De_brzina_R) * Kd_brzina)-sample_R16;
